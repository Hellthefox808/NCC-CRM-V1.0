...
5. **`backend/lib/redis.server.ts`**:
   ...
   *Fix*: Safe typed dynamic imports `(await import(/* @vite-ignore */ "@upstash/redis" as string) as any)` and guard `if (ioredisInstance?.connect) await ioredisInstance.connect().catch(() => {});`.
6. **`frontend/hooks/useSocket.ts`**:
   `socket.connected` check where `socket` is `Socket | null`.
   *Fix*: `if (socket && socket.connected)`.
7. **`frontend/features/pages/Home.tsx`**:
   ...
   *Fix*: Change `openStatusModal: (query?: string) => void;`.
8. **`frontend/features/pages/AdminDashboard.tsx` & `CadetDashboard.tsx`**:
   ...
   *Fix*: Cast `setActiveTab as (tab: string) => void` or `(t) => setActiveTab(t as any)`.
9. **`frontend/features/Cadet/CadetDashboardOverview.tsx`**:
   ...
   *Fix*: Add optional properties to `CadetNotificationItem` and `src/types.ts` `CadetProfile`.
...
```typescript
          ...
          const channel =
            rawChannel === "SOCKET"
              ? "SOCKET_IO"
              : (rawChannel as "EMAIL" | "SOCKET_IO" | "IN_APP" | "BOTH");
          const rawScope = String(body.recipientScope || "ALL_CADETS");
          const recipientScope =
            rawScope === "OFFICERS_ONLY"
              ? "OFFICERS"
              : (rawScope as "ALL_CADETS" | "PI_STAFF" | "OFFICERS");
...
```typescript
    const { data, error } = await (admin as any)
      ...
      .select("id");
```
*(Similarly apply `(admin as any).from("calendar_event_reminders")` in `updateEventReminders`, `cancelEventReminders`, and `addCustomReminder`)*
...
```typescript
    const { data, error } = await (admin as any)
      ...
      .single();
```
*(Similarly apply `(admin as any).from("email_jobs")` and `(admin as any).from("email_delivery_logs")` throughout `queue.service.ts`)*
