...
export const NotificationsFeed: React.FC<NotificationsFeedProps> = ({
  ...
  const [selectedTab, setSelectedTab] = useState<"all" | "training" | "assignment" | "notices">("all");
  const [unreadOnly, setUnreadOnly] = useState<boolean>(false);
  const [searchQuery, setSearchQuery] = useState<string>("");

  // Handle incoming real-time broadcast via WebSocket
  const handleRealtimeNotice = useCallback((newNotice: any) => {
    const formattedNotice: NotificationItem = {
      ...
      read: false,
      priority: newNotice.priority === "CRITICAL" ? "Urgent" : newNotice.priority === "HIGH" ? "High" : "Normal",
      ...
      .then((res) => {
        if (res.success && res.data?.notifications?.length) {
          const apiItems: NotificationItem[] = res.data.notifications.map((n: any) => ({
            ...
            date: n.date,
            read: Boolean(n.read),
            priority: n.priority === "CRITICAL" ? "Urgent" : n.priority === "HIGH" ? "High" : "Normal",
  ...
  const noticesUnreadCount = useMemo(() => {
    return notifications.filter(
      (n) => !n.read && (n.category === "Parade Order" || n.category === "Camp Notice" || n.category === "System Alert")
    ).length;
  }, [notifications]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#002147] via-[#001c3d] to-[#001026] text-white rounded-3xl p-6 sm:p-8 shadow-xl border border-yellow-500/30 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center space-x-2 bg-yellow-400/15 border border-yellow-400/40 text-yellow-300 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight flex items-center space-x-3">
              ...
              {unreadCount > 0 ? (
                <span className="relative inline-flex items-center">
                  <span className="bg-gradient-to-r from-red-500 to-amber-500 text-white text-xs font-black px-3 py-1 rounded-full shadow-lg border border-white/20 flex items-center space-x-1.5">
            ...
            <p className="text-xs sm:text-sm text-slate-300 max-w-2xl font-medium">
              Live updates for battalion parade drills, weapon practice schedules, assignment due dates, and official ANO office announcements.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 shrink-0">
            <motion.button
              ...
              className="bg-yellow-400 hover:bg-yellow-300 text-slate-950 font-black px-4 py-2.5 rounded-2xl text-xs flex items-center space-x-2 shadow-lg cursor-pointer transition-all border border-yellow-300"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-6 pt-6 border-t border-white/10">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-3 flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <p className="text-[11px] text-slate-300 font-bold">Training Updates</p>
                <p className="text-xs text-slate-400">Drill & Parade</p>
              </div>
            </div>
            <span className={`text-xs font-black px-2 py-0.5 rounded-full ${trainingUnreadCount > 0 ? "bg-amber-400 text-slate-950" : "bg-white/10 text-slate-300"}`}>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-3 flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <p className="text-[11px] text-slate-300 font-bold">Assignment Reminders</p>
                <p className="text-xs text-slate-400">Quizzes & Submissions</p>
              </div>
            </div>
            <span className={`text-xs font-black px-2 py-0.5 rounded-full ${assignmentUnreadCount > 0 ? "bg-indigo-400 text-slate-950" : "bg-white/10 text-slate-300"}`}>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-3 flex items-center justify-between col-span-2 sm:col-span-1">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <p className="text-[11px] text-slate-300 font-bold">Officer Notices</p>
                <p className="text-xs text-slate-400">Parade & Camp Orders</p>
              </div>
            </div>
            <span className={`text-xs font-black px-2 py-0.5 rounded-full ${noticesUnreadCount > 0 ? "bg-red-500 text-white" : "bg-white/10 text-slate-300"}`}>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs space-y-4">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-1.5 overflow-x-auto pb-1 sm:pb-0 no-scrollbar">
            ...
            >
              <span>All Feed</span>
              <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-black ${selectedTab === "all" ? "bg-yellow-400 text-slate-950" : "bg-slate-200 text-slate-800"}`}>
          ...
          <div className="flex items-center space-x-3 shrink-0">
            ...
            <div className="relative flex-1 sm:w-64">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-yellow-400 outline-hidden"
              />
              {searchQuery && (
                <button
            ...
            {/* Unread Only Toggle */}
            <label className="flex items-center space-x-2 cursor-pointer bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl hover:bg-slate-100 transition-colors shrink-0 select-none">
              <input
                type="checkbox"
                checked={unreadOnly}
                onChange={(e) => setUnreadOnly(e.target.checked)}
                className="w-3.5 h-3.5 rounded text-[#002147] focus:ring-yellow-400 cursor-pointer"
              />
              <span className="text-xs font-bold text-slate-700">Unread Only</span>
            </label>
        ...
        <AnimatePresence>
          {filteredNotifications.length > 0 ? (
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pl-1">
                  ...
                  <div className="flex items-center space-x-2.5">
                    ...
                    {/* Category Icon Badge */}
                    <div className={`p-2 rounded-xl border flex items-center justify-center shrink-0 ${getCategoryBadgeStyle(n.category)}`}>
                      {getCategoryIcon(n.category)}
                    </div>

                    <div>
                      <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                        <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border uppercase tracking-wider ${getCategoryBadgeStyle(n.category)}`}>
                          {n.category}
                        </span>

                        <span className={`text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider ${getPriorityBadgeStyle(n.priority)}`}>
                          {n.priority}
                        </span>
                ...
                {/* Body Text */}
                <p className="text-xs sm:text-sm text-slate-700 font-medium leading-relaxed pl-1 sm:pl-10">
                  {n.body}
                </p>

                {/* Footer Metadata & Action Handler */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2.5 border-t border-slate-200/70 pl-1 sm:pl-10">
                  
                  <div className="flex items-center space-x-4 text-[11px] text-slate-500 font-medium flex-wrap gap-y-1">
                  ...
                  {n.actionLabel && (
                    <button
                      ...
                      className={`px-3.5 py-1.5 rounded-xl text-xs font-black flex items-center space-x-1.5 cursor-pointer shadow-2xs transition-all ${
                        n.category === "Training Update"
                          ? "bg-[#002147] text-yellow-400 hover:bg-[#001838]"
                          : n.category === "Assignment Reminder"
            ...
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-white rounded-2xl border border-slate-200 p-12 text-center space-y-4 shadow-xs"
            >
              ...
              <div className="space-y-1 max-w-md mx-auto">
                <h3 className="text-lg font-black text-slate-900">No Notifications Found</h3>
              ...
              {(unreadOnly || searchQuery) && (
                <button
                  ...
                  className="bg-[#002147] text-yellow-400 hover:bg-[#001838] font-black px-4 py-2 rounded-xl text-xs inline-flex items-center space-x-2 cursor-pointer"
                >
