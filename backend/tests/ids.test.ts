...
import testSuite from "node:test";
...
testSuite("Intrusion Detection System (IDS) Unit Tests", () => {
  testSuite.it("getAlertLevelForScore() correctly maps numerical scores to risk levels", () => {
  ...
  testSuite.it("IDS_RULES defines complete threat triggers for key security events", () => {
  ...
  testSuite.it(
    "recordSecurityEvent() records events, calculates cumulative risk, and triggers alerts",
    async () => {
  ...
  testSuite.it(
    "recordSecurityEvent() immediately triggers CRITICAL alert for unauthorized export attempt",
    async () => {
